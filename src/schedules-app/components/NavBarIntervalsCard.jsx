...
const NavBarIntervalsCard = ({ playingSession, playingLoop }) => {
  ...
  lastRecord = useSelector(state => state.apps.timerRecords.newestTimerRecord)
  ...
  useEffect(() => {
    return () => {
      if (isPlaying) {
        ...
        sendTimerState(playingTime, 'stop', recordInterval)
        // sendTimerState(playingTime, 'stop: session card', recInterval)
  ...
  useEffect(() => {
    window.addEventListener('beforeunload', e => {
      ...
      if (isPlaying) {
        ...
        toast(`${currentInterval.name} stopped`)
        sendTimerState(playingTime, 'stop', recordInterval)
        // sendTimerState(playingTime, 'stop: reload', recInterval)
        if (intervalIsSet) workerTimers.clearInterval(intervalIsSet)
      }
    })
  }, [])

  const sendTimerState = (timerState, playState, interval) => {
    if (lastRecord) {
      const timeRecord = {
      ...
      dispatch(updateTimerRecords(timeRecord, lastRecord._id))
  ...
  const start = () => {
    ...
    setPlay(true)
    run()
    sendTimerState(time, 'start', currentInterval)
  ...
  const stop = () => {
    ...
    sendTimerState(time, 'stop', currentInterval)
  ...
  const handleStepForward = () => {
    if (intervalIndex < intervals.length - 1) {
      ...
      if (play) {
        ...
        toast(`${currentInterval.name} stopped`)
        sendTimerState(time, 'stop', currentInterval)
        // sendTimerState(time, 'stop: step forward', currentInterval)
        workerTimers.clearInterval(interv)
      }
  ...
  const handleStepBackward = () => {
    if (intervalIndex > 0) {
      ...
      if (play) {
        ...
        toast(`${currentInterval.name} stopped`)
        sendTimerState(time, 'stop', currentInterval)
        // sendTimerState(time, 'stop: step backward', currentInterval)
        workerTimers.clearInterval(interv)
      }
